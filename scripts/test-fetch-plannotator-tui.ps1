...
try {
  ...
  if ($Case -eq "LocalOverride") {
    ...
    Assert-True ((Get-Content -LiteralPath $stamp -Raw) -ceq "0.9.0") "local stamp differs"
    ...
    try {
      ...
      Assert-True ((Get-Content -LiteralPath $stamp -Raw) -ceq "0.9.0") "download stamp differs"
