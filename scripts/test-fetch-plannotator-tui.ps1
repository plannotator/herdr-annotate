...
try {
  ...
  if ($Case -eq "LocalOverride") {
    ...
    Assert-True ((Get-Content -LiteralPath $stamp -Raw) -ceq "0.9.3") "local stamp differs"
    ...
    try {
      ...
      Assert-True ((Get-Content -LiteralPath $stamp -Raw) -ceq "0.9.3") "download stamp differs"
