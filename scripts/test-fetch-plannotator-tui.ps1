...
try {
  ...
  if ($Case -eq "LocalOverride") {
    ...
    Assert-True ((Get-Content -LiteralPath $stamp -Raw) -ceq "0.9.4") "local stamp differs"
    ...
    try {
      ...
      Assert-True ((Get-Content -LiteralPath $stamp -Raw) -ceq "0.9.4") "download stamp differs"
